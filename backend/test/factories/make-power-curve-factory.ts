...
export class MakePowerCurve {
  async toDomain({
    ...
    const createPowerCurveUseCase = await new CreatePowerCurveUseCase(inMemoryPowerCurveRepository).execute({
  ...
  async toHTTP({file, name}: {name?: string, file?: string}): Promise<request.Test> {
    const filePath = resolve(__dirname, '..', 'files/example-Abr-2017-curva-potencia-windbox.csv')
