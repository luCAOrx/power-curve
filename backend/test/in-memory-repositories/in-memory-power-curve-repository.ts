...
export class InMemoryPowerCurveRepository implements PowerCurveRepository {
  ...
  async exists(name: string): Promise<boolean> {
    return this.powerCurves.some(powerCurve => powerCurve.props.name === name)
  }
  ...
  async findMany(page: number, takePage: number): Promise<PowerCurve[]> {
    const powerCurvesOrPowerCurve = this.powerCurves.map(powerCurves => powerCurves)

    return powerCurvesOrPowerCurve.slice((page - 1) * takePage, page * takePage)
  }
}
