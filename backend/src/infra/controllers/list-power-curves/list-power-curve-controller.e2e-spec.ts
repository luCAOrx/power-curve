...
beforeAll(async () => {
  for (let i = 0; i < 30; i++) {
    ...
    const testFilePath = resolve(__dirname, '..', '..', '..', '..', 'test', 'uploads')
...
afterAll(() => {
  testUploadsDirectory.map(file => {
    const fileDir = resolve(__dirname, '..', '..', '..', '..', 'test', 'uploads', `${file}`)
...
describe('List Power Curves Controller', () => {
  it('should be able to list power curves', async () => {
    const {body, statusCode} = await new MakePowerCurveList().toHTTP({page: 1, takePage: 5})

    makeArrayWithFivePowerCurvesObjects({responseBody: body}).map(powerCurveOrPowerCurves => {
  ...
  it('should be able to paginate', async () => {
    const {
      body: pageOneBodyResponse,
      statusCode: pageOneStatusCode
    } = await new MakePowerCurveList().toHTTP({page: 1, takePage: 5})

    makeArrayWithFivePowerCurvesObjects({responseBody: pageOneBodyResponse}).map(powerCurveOrPowerCurves => {
    ...
    const {
      body: pageTwoBodyResponse,
      statusCode: pageTwoStatusCode
    } = await new MakePowerCurveList().toHTTP({page: 2, takePage: 5})

    makeArrayWithFivePowerCurvesObjects({responseBody: pageTwoBodyResponse}).map(powerCurveOrPowerCurves => {
